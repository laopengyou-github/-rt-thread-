...
void sensor_show_data(rt_size_t num, rt_sensor_t sensor, struct rt_sensor_data *sensor_data)
{
    ...
    case RT_SENSOR_CLASS_TEMP:
        LOG_I("temp:%3d.%d C",sensor_data->data.temp / 10, sensor_data->data.temp % 10);
        break;
    case RT_SENSOR_CLASS_HUMI:
        LOG_I("humi:%3d.%d%%",sensor_data->data.humi / 10, sensor_data->data.humi % 10);
        break;
...
void sensor(int argc, char **argv)
{
    ...
    else if (!strcmp(argv[1], "read"))//if argv[1]'s strings = read ,execute the follow code
    {                                 //ËùÒÔ²Â²â£¬a[0]=sensor
        rt_uint16_t num = 1;
        ...
        if (argc == 3) //array's length is 3
        {
            num = atoi(argv[2]);
        }

        sensor = (rt_sensor_t)dev;
        //delay  = sensor->info.period_min > 100 ? sensor->info.period_min : 100;//sensor->info.period_min > 100 ? sensor->info.period_min : 100;
    ...
    {
        if (!strcmp(argv[1], "probe"))//if argv[1]'s strings = probe ,execute the follow code
        {
            rt_uint8_t reg = 0xFF;
            if (dev)
            {
