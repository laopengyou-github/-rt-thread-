...
* 2.1 通过温湿度传感器进行温湿度采集，将采集到的数据通过串口发送温湿度到上位机，并利用wifi模块将温湿度上传到云端，在手机端可以通过onenet云查看家中温湿度。

* 2.2 利用OLED屏幕实时显示温湿度，便于在家中查看。

* 2.3 如果温度过高，则电机（风扇）开启进行降温，湿度过低则，水泵开启为花卉浇水。

* 2.4 云端控制4种颜色led灯的开关，实现光源供应。

### 三、RT-Thread使用情况概述：

* 3.1 内核方面

  （1）使用了线程，邮箱实现任务调度。具体创建了三个线程分别进行温度的检测与上传，湿度的检测与上传和OLED显示数据。

  （2）使用两个邮箱，一个在"温度检测与上传"线程中用来传递温度值给"OLED显示数据"的线程，另一个在"湿度检测与上传"线程中用来传递湿度值给"OLED显示数据"的线程。

* 3.2 软件包方面

  （1）aht10传感器驱动库：实现AHT10驱动，在应用层调用测量温湿度。

  （2）u8g2不同种类单色屏驱动库：实现OLED驱动，在应用层调用显示温湿度。

  （3）cJSON: 超轻量级的 C 语言 json 解析库。

  （4）OneNET连接中国移动 OneNet 云的软件包：实现硬件连接云端，在应用层调用上传温湿度，以及下发命令控制硬件。

  （5）AT DEVICE：AT 组件在不同设备上的移植或示例。

  （6）WebClient(RT-Thread 官方开源的 http/https 协议客户端)。
...
![整体系统框架](1.picture/image-20220803153659173.png)
...
![硬件框架](1.picture/image-20220803153506362.png)

### 六、软件框架说明：

![软件框架](1.picture/image-20220803153007336.png)

* 6.1 外围设备与传感器
  * 6.1.1主要介绍了使用到的一些硬件模块
* 6.2 硬件接口及驱动层
  * 6.2.1 主要介绍了使用到的一些板载硬件资源
  * 6.2.2 接口使用说明
    * I2C接口接AHT10
    ...
    * UART接口接ESP8266
* 6.3 RTThread系统层
  * 6.3.1 主要介绍了使用的RTThread内核以及工具
* 6.4 应用层
  * 6.4.1 主要介绍该工程的具体功能

### 七、软件模块说明:

![软件模块](1.picture/image-20220803153222340.png)
